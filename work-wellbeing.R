...
xlab = "Weekly Hours of Work/ 10"  ## Weekly hours devided by 10
...
ylim = c(-.3, .3)  # SET AS YOU LIKE -- here, how much movement across a standard deviation unit of the outcome
...
data_raw$Gender3 <- factor(data_raw$Gender3, labels = c("Female", "Neither Male Nor Female", "Male"))
data_raw$SexualOrientation <- factor(data_raw$SexualOrientation, labels =c ("Heterosexual", "Gay/Lesbian", "Bisexual"))
data_raw$Ethnicity<- factor(data_raw$EthCat, labels = c("European", "Maori", "Pacific", "Asian"))
data_raw$Retired <- factor(data_raw$retired, labels = c("No", "Yes"))
data_raw$Employed<-factor(data_raw$Employed, labels = c("Unemployed", "Employed"))
data_raw$HLTH.Disability <- factor (data_raw$HLTH.Disability, labels = c ("Non-disabled", "Disabled"))
...
table1::table1(
  ~
    ...
    #Alcohol.Intensity_lead2+
    Alcohol.Intensity_log_lead2+
    Smoker_lead2+
    HLTH.BMI_lead2+
    #Hours.Exercise_lead2+
    Hours.Exercise_lead2_log+
    HLTH.SleepHours_lead2+
    ...
    GRATITUDE_lead2+
    ImpermeabilityGroup_lead2+
    PermeabilityIndividual_lead2+
    LIFESAT_lead2+
    LIFEMEANING_lead2+
    PERFECTIONISM_lead2+
    PWI_lead2+
...
mean(data_raw$Hours.Exercise_lead2)
mean(data_raw$Hours.Exercise_lead2_log)
data_raw$Alcohol.Frequency_lead2
data_raw$Alcohol.Frequency_lead2ord

# n ids
length(1:nrow(data_imputed$data))
...
sub = "How many drinks containing alcohol do you have on a typical\nday when drinking?"
...
main = "Log Hours of Exercise"
ylab = "Log Hours of Exercise (SD)"
...
main = "Impermeability of Group"
ylab = "Impermeability of Group (SD)"
sub = "The current income gap between\nNew Zealand Europeans and other ethnic\ngroups would be very hard to change."
...
sub = "Doing my best never seems to be enough.\nMy performance rarely measures up to\nmy standards.\nI am hardly ever satisfied with my\nperformance."
...
sub = "Other people have too much power or\ncontrol over important parts of my life."
...
main = "Vengefulness (anti-Forgiveness)"
ylab = "Vengefulness (anti-Forgiveness) (SD)"
sub = "Sometimes I can't sleep because of thinking\nabout past wrongs I have suffered.\nI can usually forgive and forget when\nsomeone does me wrong.\nI find myself regularly thinking about past\ntimes that I have been wronged."
...
# Work-life balance -------------------------------------------------------
# note-- we have no measure currently at baseline, so less confounding control
...
main = "Log Income"
ylab = "Log Income (SD)"
sub = "Please estimate your total household income (before tax) for\nthe last year."
# regression
out_m <- mice_gaussian(df = df, X = X, Y = Y, cvars=cvars)
...
# OLS (continuous)
out_t <- bind_forestplot(list(sfhealth_p, 
                              alcoholfreq_p, 
                              alcoholintensity_p, 
                              bmi_p,
                              exercise_p,
...
gcomp_forestplot(out_t, title = "Outcome-Wide Work", 
                 ylim = ylim, xlab = "Change in Hours of Work (SD)")

## Risk ratio plot (binary)
out_tt <- bind_forestplot(list(smoker_p, volunteers_p,homeowner_p))
gcomp_forestplot_rr(out_tt, title = "Outcome-Wide Work RR",
                    ylim = c(.5,1.5))
...
main = "Health outcome estimands / E-values"
h_tab <- rbind(sfhealth_c,
               alcoholfreq_c,
               alcoholintensity_c,
               smoker_c,
               bmi_c,
...
h_tab |>
  kbl(caption = main,
      ...
      "html") |>
  kable_styling() %>%
  row_spec(c(1,3,4,5,6,7),  # Bold out the lines where EVALUES do not cross zero or for ratios, 1
           bold = T,
           # color = "black",
           background = "bold")
...
main = "Embodied wellbeing estimands / E-values"
embody_tab <- rbind(
  bodysat_c,
  distress_c,
  #exercise_c,
...
main = "Reflective wellbeing estimands / E-values"
reflect_tab <- rbind(
  gratitude_c,
  groupimperm_c,
  selfperm_c,
...
main = "Social wellbeing estimands / E-values"
...
main = "Economic wellbeing estimands / E-values"
econ_tab <- rbind(
  income_c,
  charity_c,
  homeowner_c,
...
embody_plots <-
  ...
  selfcontrol_p +
  #sleep_p +
  sexualsat_p + plot_annotation(title = "Causal effects of working hours on embodied wellbeing", #subtitle = "xyz",
                                tag_levels = "A") +
...
health_plots <- sfhealth_p +
  ...
  exercise_p +
  sleep_p+
  plot_annotation(title = "Causal effects of working hours on health outcomes",
                  # subtitle = "xyz",
...
reflective_plots <- gratitude_p +
  ...
  veng_p +
  worklife_p+
  plot_annotation(title = "Causal effects of working hours on reflective wellbeing", 
                  tag_levels = "A")+
...
social_plots <- belong_p +
  ...
  nwi_p +
  support_p + 
  volunteers_p+plot_annotation(title = "Causal effects of working hours on social wellbeing",
                              tag_levels = "A") +
  plot_layout(guides = 'collect') #+ plot_layout(nrow = 3, byrow = T)
...
econ_plots <-
  ...
  nzsei_p +
  standardliving_p +
  plot_annotation(title = "Causal effects of working hours on economic wellbeing",
                  tag_levels = "A") +
  plot_layout(guides = 'collect') #+ plot_layout(nrow = 3, byrow = T)
...
fatigue_p_raw <- ggplot_stglm(out_ct2, ylim = ylim, main, xlab, ylab, min = min, p=p, sub = sub)
fatigue_p_raw
...
library(lme4)
data_raw1 <- data_raw |>
  mutate(HLTH.Fatigue_lead2_z = scale(HLTH.Fatigue_lead2, scale = TRUE, center = TRUE),
         Hours.Work_lead1_10 = Hours.Work_lead1/10
  )

predict_fatigue <- glm(HLTH.Fatigue_lead2_z ~  bs(Hours.Work_lead1_10), data = data_raw1 )


fatigue_BAD <- plot(
  ggeffects::ggpredict(predict_fatigue, terms = "Hours.Work_lead1_10[0:8]")
) + scale_y_continuous(limits = ylim) + theme_classic()

fatigue_BAD
fatigue_BAD +fatigue_p_raw
